...
void
nem_partitionElts_Match (struct IN In, struct MESH *pMesh, struct PART *pPart)
{
...
int
nem_partitionElts_Balancing (struct MESH *pMesh, struct PART *pPart, double level)
{
  int i;
  int **Q = NULL;
  int BoundEltQty;
  ...
  sum = nem_partitionElts_Balancing_Q (*pMesh, pPart, level, BoundEltQty, Q);
...
void
nem_partitionElts_ByNumber (struct IN In, struct MESH *pMesh, struct PART *pPart)
{
  int j;
