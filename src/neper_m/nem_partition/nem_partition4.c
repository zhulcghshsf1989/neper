...
int
nem_partitionElts_Balancing_Q (struct MESH Mesh, struct PART *pPart,
			   double level, int BoundEltQty, int **Q)
{
  ...
  for (i = 0; i < BoundEltQty; i++)
    nem_partitionElts_Balancing_Q_elt (Mesh, *pPart, A, Q[i]);
  ...
  while (sum != 0 && iter < 100000 * (double) sum / inisum)
  {
    ...
    for (i = 0; i < qty; i++)
    {
      ...
      oldpart = Q[0][1];
      nem_partitionElts_Balancing_Q_elt (Mesh, *pPart, A, Q[0]);
      newpart = Q[0][1];
	...
	for (j = 0; j < BoundEltQty; j++)
	  if (Q[j][2] == oldpart || Q[j][2] == newpart)
	    nem_partitionElts_Balancing_Q_elt (Mesh, *pPart, A, Q[j]);
	iter = -1;
