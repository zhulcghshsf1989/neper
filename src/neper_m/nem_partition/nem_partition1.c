...
void
nem_partition (struct IN In, struct NODES *pNodes, struct MESH *pMesh0D,
	   ...
	   struct PART *pPart)
{
  ...
  ut_print_message (0, 2, "nem_partitioning nodes ...\n");
  nem_partitionNodes (In, &SCMesh, &Arch, pNodes, pMesh0D, pMesh1D, pMesh2D,
		  pMesh, pPart);

  ut_print_message (0, 2, "nem_partitioning elements ...\n");
  nem_partitionElts (In, pMesh, pPart);

  ut_print_message (0, 2, "nem_partition properties:\n");
  nem_partitionStats (In.partqty, *pMesh, *pPart);
