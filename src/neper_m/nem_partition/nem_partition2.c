...
void
nem_partitionNodes (struct IN In, SCOTCH_Mesh * pSCMesh, SCOTCH_Arch * pArch,
...
void
nem_partitionElts (struct IN In, struct MESH *pMesh, struct PART *pPart)
{
  int i, j, qty, partqty;
  ...
  if (strcmp (In.partmethod, "none") != 0)
    {
      ...
      nem_partitionElts_Match (In, pMesh, pPart);
      if (In.partbalancing > 0)
	{
	  ut_print_message (0, 3, "Balancing partitions ... ");
	  nem_partitionElts_Balancing (pMesh, pPart, In.partbalancing);
	}
    }
  else if (strcmp (In.partmethod, "none") == 0)
    {
      ut_print_message (0, 3, "Direct partitioning by numbers ...\n");
      nem_partitionElts_ByNumber (In, pMesh, pPart);
    }
...
void
nem_partitionStats (int partnbr, struct MESH Mesh, struct PART Part)
{
  int i, cutqty;
