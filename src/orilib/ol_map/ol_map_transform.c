...
ol_map_transformparam (struct OL_MAP Map, double **F, size_t * pxsize, size_t * pysize, double *S)
{
  ...
  C[1][0] = F[0][0] * (double) (Map.xsize - 1);
  C[1][1] = F[1][0] * (double) (Map.xsize - 1);

  C[2][0] = F[0][1] * (double) (Map.ysize - 1);
  C[2][1] = F[1][1] * (double) (Map.ysize - 1);

  C[3][0] = F[0][0] * (double) (Map.xsize - 1)
    + F[0][1] * (double) (Map.ysize - 1);
  C[3][1] = F[1][0] * (double) (Map.xsize - 1)
    + F[1][1] * (double) (Map.ysize - 1);
...
ol_map_transform (struct OL_MAP Map, double **F, size_t xsize, size_t ysize, double *S, struct OL_MAP *pMap2)
{
  ...
  (*pMap2) = ol_map_alloc ((*pMap2).xsize, (*pMap2).ysize, (*pMap2).stepsize);
