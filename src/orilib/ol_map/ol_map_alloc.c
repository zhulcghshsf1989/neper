...
struct OL_MAP
ol_map_alloc (size_t xsize, size_t ysize, double stepsize)
{
  unsigned int i, j, k;
  struct OL_MAP Map;
...
ol_map_fscanf (FILE * file, struct OL_MAP *pMap)
{
  ...
  (*pMap) = ol_map_alloc ((*pMap).xsize, (*pMap).ysize, (*pMap).stepsize);
...
ol_map_fscanf_ch5 (FILE * file, struct OL_MAP *pMap)
{
  ...
  ut_file_skip (file, 1);
  for (j = 0; j < 2; j++)
    coo[0][j] = ut_file_readwcomma (file);
  ut_file_skip (file, 9);

  ut_file_skip (file, 1);
  for (j = 0; j < 2; j++)
    coo[1][j] = ut_file_readwcomma (file);
  ut_file_skip (file, 9);
