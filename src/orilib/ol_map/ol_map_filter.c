...
void
ol_map_filter_kuwahara_iter (struct OL_MAP Map, size_t size, char* crysym,
			     struct OL_MAP *pMap2, double *pdis)
{
  ...
  do
  {
    ol_map_filter_kuwahara (Map3, size, crysym, pMap2, pdis);
...
void
ol_map_filter_kuwahara (struct OL_MAP Map, size_t size, char* crysym,
                        struct OL_MAP *pMap2, double *pdis)
{
  size_t i, j;
  int qty, skip;
  ...
  for (i = skip; i < (*pMap2).xsize - skip; i++)
	...
	ol_map_filter_kuwahara_region (SubMap, crysym, (*pMap2).q[i][j], &tmpdis);
	qty++;
	(*pdis) += tmpdis;
      }
  (*pdis) /= qty;
...
void
ol_map_filter_kuwahara_region (struct OL_MAP Map, char* crysym, double *q, double *pdis)
{
  int id;
  size_t i;
  ...
  SubMap = ol_map_alloc (subsize, subsize, Map.stepsize);

  for (i = 0; i < 4; i++)
  {
    ol_map_submap (Map, coo[i][0], coo[i][1], subsize, subsize, &SubMap);
    ol_map_set (SubMap, &SubSet);
    ol_set_mean_ref (SubSet, crysym, qref, mean[i]);
    ol_set_disori (SubSet, crysym, mean[i], &(dis[i]));
    ol_set_free (SubSet);
