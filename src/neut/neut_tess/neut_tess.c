...
/* neut_tess_set_zero: set a tess to 0 / NULL */
void
...
void
neut_tess_domainface (struct TESS Tess, int **Face)
{
  int i, j;
  int face;
...
void
neut_tess_domainfacever (struct TESS Tess, int **Ver)
{
  int i, j, k;
  int face;
  ...
  neut_tess_domainface (Tess, DFace);
...
neut_tess_edge_true (struct TESS Tess, int *TruePoly, int nb)
{
  ...
  neut_tess_edge_poly (Tess, nb, poly);
...
void
neut_tess_edge_poly (struct TESS Tess, int edge, int* poly)
{
  int i, ver1, ver2;
...
neut_tess_volume (struct TESS Tess, double* pvol)
{
  ...
  {
    neut_tess_poly_volume (Tess, i, &poly_vol);
    (*pvol) += poly_vol;
  }
