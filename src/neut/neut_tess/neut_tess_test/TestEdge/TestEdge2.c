...
int
TestEdgeLength (struct TESS Tess, int i, int verbosity)
{
  double eps = 1e-15;
...
int
TestEdgeReciprocityVer (struct TESS Tess, int i, int verbosity)
{
  int j;
  int ver;
...
int
TestEdgeReciprocityFace (struct TESS Tess, int i, int verbosity)
{
  int j;
  int face;

  /* this test could be more accurate (considering edge ver bounds) */
  if (Tess.EdgeFaceQty[i] < 2)
  {
    if (verbosity)
  ...
  for (j = 0; j < Tess.EdgeFaceQty[i]; j++)
  {
    ...
    if (ut_array_1d_int_eltpos (Tess.FaceEdgeNb[face] + 1, Tess.FaceVerQty[face], i)
	== -1)
    {
      if (verbosity)
