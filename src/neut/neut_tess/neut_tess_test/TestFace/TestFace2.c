...
int
TestFaceReciprocityEdge (struct TESS Tess, int i, int verbosity)
{
  int j;
  int edge;
...
int
TestFaceReciprocityVer (struct TESS Tess, int i, int verbosity)
{
  int j;
  int ver;
  int faceqty;
...
int
TestFaceReciprocityPoly (struct TESS Tess, int i, int verbosity)
{
  int j;
  int poly;
...
int
TestFaceState (struct TESS Tess, int i, int verbosity)
{
  int j, ver;
 ...
 */
int
TestFaceBound (struct TESS Tess, int face, int verbosity)
{
  int i, ver, domver;
  ...
  for (i = 1; i <= Tess.FaceVerQty[face]; i++)
  {
    ...
    else if (Tess.VerDom[ver][0] == 0)
    {
      ...
      bound1 = ut_realloc_1d_int (bound1, bound1[0] + 1 + Tess.DomVerEdgeQty[domver]);
      ut_array_1d_int_memcpy (bound1 + bound1[0] + 1, Tess.DomVerEdgeQty[domver],
...
int
TestFaceSelfIntersect (struct TESS Tess, int face, int verbosity)
{
  int i;
  int res = 0;
  ...
  for (i = 0; i < Tess.FaceVerQty[face]; i++)
  {
    ...
    ut_space_projpoint_alongonto (coo[i], Tess.FaceEq[face] + 1, Tess.FaceEq[face]);
  }
