 ...
 */
int
TestVerEdgeQtyNNb (struct TESS Tess, int i, int verbosity)
{
  int j;
  int edge;
  int res;

  if (Tess.VerEdgeQty[i] < 3)
  {
    if (verbosity)
...
int
TestVerEdgeReciprocity (struct TESS Tess, int i, int verbosity)
{
  int j;
  int edge;
...
int
TestVerBoundNCoo (struct TESS Tess, int ver, int verbosity)
{
  int i;
  int qty;
  int* face = NULL;
  ...
  for (i = 0; i < qty; i++)
  {
    ut_space_point_plane_dist (Tess.VerCoo[ver], Tess.DomFaceEq[face[i]], &dist);
    if (dist > 1e-8)
    {
      if (verbosity)
...
int
TestVerFaceCoplaneity (struct TESS Tess, int ver, int verbosity)
{
  int i, j, k;
  int edge1, edge2;
  int face;
