...
neut_tess_test_dom_def (struct TESS Tess, int verbosity)
{
  ...
  for (i = 1; i <= Tess.DomEdgeQty; i++)
  {
    for (j = 0; j < 2; j++)
    {
      ...
      if (ut_array_1d_int_eltpos (Tess.DomVerEdgeNb[ver], Tess.DomVerEdgeQty[ver], i) == -1)
      {
	if (verbosity)
    ...
    for (j = 0; j < 2; j++)
    {
      ...
      if (ut_array_1d_int_eltpos (Tess.DomFaceEdgeNb[face] + 1, Tess.DomFaceVerQty[face], i) == -1)
      {
	if (verbosity)
