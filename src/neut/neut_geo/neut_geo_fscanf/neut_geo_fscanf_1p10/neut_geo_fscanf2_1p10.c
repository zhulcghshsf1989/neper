...
neut_geo_fscanf_head_1p10 (struct GEO* pGeo, FILE * file)
{
  ...
  {
    ut_file_skip (file, 1);
    (*pGeo).PolyId = ut_alloc_1d_int ((*pGeo).N + 1);

    // TODO: change to PolyQty for non-standard tessellations
    for (i = 1; i <= (*pGeo).N; i++)
      if (fscanf (file, "%d", &((*pGeo).PolyId[i])) != 1)
	abort ();
  }
  ...
  {
    ut_file_skip (file, 1);
    if (fscanf (file, "%lf%lf%s", &((*pGeo).maxff), &((*pGeo).sel), tmp) != 3)
      abort ();

    if (strcmp (tmp, "nobound") != 0)
    {
      strcpy ((*pGeo).dbound, tmp);
      if (fscanf (file, "%lf", &((*pGeo).dboundsel)) != 1)
	abort ();
    }
  }
