...
void
nefm_init_remeshingmapping_geo_updating (struct GEO* pGeo, struct NODES
...
void
cl2sel (struct GEOPARA* pGeoPara)
{
  /* Calculation of sel */
  if ((*pGeoPara).sel < 0)
  {
    (*pGeoPara).sel = (*pGeoPara).cl / (*pGeoPara).pcl;
    if ((*pGeoPara).rsel > 0)
    (*pGeoPara).sel *= (*pGeoPara).rsel;
  }

  /* Calculation of dboundsel */
  if ((*pGeoPara).dbound != NULL && (*pGeoPara).dboundcl > 0)
    if ((*pGeoPara).dboundsel < 0)
    {
      (*pGeoPara).dboundsel = (*pGeoPara).dboundcl / (*pGeoPara).dboundpcl;
      if ((*pGeoPara).dboundrsel > 0)
	(*pGeoPara).dboundsel *= (*pGeoPara).dboundrsel;
      
      if ((*pGeoPara).dbound == NULL)
	ut_print_messagewnc (1, 72,
		    "While using option `-dbound[r]cl', you should also use option `-dbound'. Otherwise, `-dbound[r]cl' has no effect ...");
    }
