		  ...
		  struct MESH* pRMesh2D, struct MESH* pRMesh3D)
{
  ...
  else
  {
    ...
    nem_init_remeshingmapping_geo_updating (pGeo, *pRNodes, *pRMesh0D, *pRMesh1D, *pRMesh2D, *pRMesh3D);
  }
...
void
nem_init_scaling (struct GEO* pGeo, struct VOX* pVox, struct NODES* pRNodes,
    ...
    struct MESH RMesh3D, struct GEOPARA* pGeoPara)
{
  ...
  if ((*pGeoPara).cltype == 1)
    rcl2cl ((*pGeoPara).rcl, vol, polyqty, &((*pGeoPara).cl));
