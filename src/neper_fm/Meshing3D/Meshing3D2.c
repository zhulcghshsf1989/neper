...
int
Meshing3D_poly_cl (struct GEOPARA GeoPara, struct GEO Geo, int poly,
                double* pcl)
{
...
int
Meshing3D_poly (struct IN In, double cl, struct MULTIM* pMultim,
		...
		struct MESH *pMesh3D, int id)
{
  ...
  for (a = 0; a < (*pMultim).algoqty; a++)
  {
    ...
    for (iter = 0; iter < In.mesh3diter; iter++)
    {
      ...
      status = nefm_mesh_3d_gmsh (Geo, id, *pNodes, Mesh2D, cl,
  ...
  for (i = 0; i < skinnodes_qty; i++)
  {
    ...
    {
      ut_print_message (2, 0, "Meshing3D: skin node not found or bad node_nbs\n");
      abort ();
    }
