...
void
nefm_mesh_3d_improve_stellar (struct NODES* pN, struct MESH* pM, int skinnodeqty)
{
  int i, j, id, qty;
  FILE* file;
  FILE* file2;
  ...
  nefm_mesh_fprintf_stellar_config (file);
  ut_file_close (file, "Stellar.config", "W");

  file = ut_file_open ("tmp.node", "W");
  nefm_mesh_fprintf_stellar_nodes (file, skin, *pN);
  ut_file_close (file, "tmp.node", "W");
  file = ut_file_open ("tmp.ele", "W");
 ...
 * renumber accordingly */
  ...
  for (i = 1; i <= skinnodeqty; i++)
  {
    ...
    {
      ut_print_message (2, 0, "nefm_mesh_3d_improve_stellar: skin node not found\n");
      abort ();
    }
    node_nbs[i] = id;
    node_nbs[0]++;
...
void
WriteMeshStellar (struct IN In, struct NODES Nodes, int* BoundNodes, struct MESH Mesh3D)
{
  FILE *file;

  file = ut_file_open (In.stenode, "w");
  nefm_mesh_fprintf_stellar_nodes (file, BoundNodes, Nodes);
  ut_file_close (file, In.stenode, "w");

  file = ut_file_open (In.steele, "w");
  nefm_mesh_fprintf_stellar_elts (file, Mesh3D);
  ut_file_close (file, In.steele, "w");
