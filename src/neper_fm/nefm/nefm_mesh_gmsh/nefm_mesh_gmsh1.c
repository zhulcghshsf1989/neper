...
int
nefm_mesh_2d_gmsh (struct GEO Geo, int face, double *face_proj,
 ...
 * writing nodes then 1D elements */
  ...
  nefm_mesh_2d_gmsh_writenodes (Geo, Nodes, Mesh1D, face, face_proj, file);
  nefm_mesh_2d_gmsh_write1dmesh (Geo, Mesh1D, face, file);

/*********************************************************************** 
 * writing face */

  nefm_mesh_2d_gmsh_writeface (Geo, Mesh1D, face, cl, file);
...
int
nefm_mesh_2d_gmsh_b (struct GEO Geo, int face,
...
// writing boundary = "OD elements = *pNodes", then 1D elements
  ...
  WriteMeshGmsh (file2, *pNodes, Skin, Garbage, Garbage);
...
// writing face
  ...
  nefm_mesh_gmsh_options (file, nb, 4, 1, rnd);
  nefm_mesh_2d_gmsh_writeface_b (-1, file);
  ut_file_close (file, "tmp.geo", "W");
*/

int
nefm_mesh_3d_gmsh (struct GEO Geo, int poly, struct NODES Nodes,
