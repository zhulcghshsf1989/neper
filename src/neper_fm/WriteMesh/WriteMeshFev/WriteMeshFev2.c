...
void
neut_mesh_fprintf_fev_parms (FILE* file, struct NODES Nodes, struct MESH Mesh)
{
  /* fev1 = nb of elts and nodes */
  fprintf (file, "%d %d 9\n", Mesh.EltQty, Nodes.NodeQty);
}

void
neut_mesh_fprintf_fev_nodes (FILE * file, struct NODES Nodes)
{
  int i, j;
...
void
neut_mesh_fprintf_fev_elsets (FILE * file, struct MESH Mesh3D)
{
  int i;
...
void
neut_mesh_fprintf_fev_nsets (FILE* file, struct NSET NSet0D, struct NSET NSet1D,
                             struct NSET NSet2D, char* nset)
{
  ...
  for (i = 0; i < qty; i++)
  {
    ...
    for (j = 1; j <= NSet0D.qty; j++)
      if (! strcmp (name[i], NSet0D.names[j]))
      {
	neut_mesh_fprintf_fev_nset (file, NSet0D.names[j], NSet0D.nodeqty[j], NSet0D.nodes[j]);
	status = 0;
	break;
      }
    ...
    for (j = 1; j <= NSet1D.qty; j++)
      if (! strcmp (name[i], NSet1D.names[j]))
      {
	neut_mesh_fprintf_fev_nset (file, NSet1D.names[j], NSet1D.nodeqty[j], NSet1D.nodes[j]);
	status = 0;
	break;
      }
    ...
    for (j = 1; j <= NSet2D.qty; j++)
      if (! strcmp (name[i], NSet2D.names[j]))
      {
	neut_mesh_fprintf_fev_nset (file, NSet2D.names[j], NSet2D.nodeqty[j], NSet2D.nodes[j]);
	status = 0;
	break;
      }
