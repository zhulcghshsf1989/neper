...
PlanePara (int center, int nei, struct GERMSET GermSet, double *plane)
{
  ...
  for (i = 0; i < 3; i++)
    plane[i + 1] = 2 * (GermSet.GermCoo[nei][i] - GermSet.GermCoo[center][i]);

  plane[0] = 0;
  for (i = 0; i < 3; i++)
    plane[0] +=
      (pow (GermSet.GermCoo[nei][i], 2) -
       pow (GermSet.GermCoo[center][i], 2));
 ...
 */
void
BadVerDet (double *plane, int CenterSide, struct POLYMOD Polymod, int **pBadVer)
{
  /* Mute variables */
  int i;
