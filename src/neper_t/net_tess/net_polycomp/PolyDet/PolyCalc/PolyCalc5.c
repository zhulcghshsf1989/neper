...
UpdFaceEq (struct POLYMOD *pPolymod, double *plane)
{
  ...
  (*pPolymod).FaceEq = ut_realloc_2d_addline ((*pPolymod).FaceEq, (*pPolymod).FaceQty + 1, 4);
...
{
  (*pPolymod).VerUse = ut_realloc_1d_int ((*pPolymod).VerUse, (*pPolymod).VerQty + 1);
...
UpdFaceVerNb (struct POLYMOD *pPolymod)
{
  ...
  (*pPolymod).FaceVerNb[(*pPolymod).FaceQty][(*pPolymod).FaceVerQty[(*pPolymod).FaceQty]]
    = (*pPolymod).VerQty;
...
NeiVer (struct POLYMOD Polymod, int face, int sum, int near)
{
  ...
  NeiPos =
    oneDIntEltPos (Polymod.FaceVerNb[face], 1, Polymod.FaceVerQty[face], sum,
		   near);
		 ...
		 int *pNbBadVer)
{
  ...
  return oneDIntEltPos (faces2, 0, 1, pFace, 0);
}
		    ...
		    int *faces2)
{
  ...
  (*pPolymod).FaceVerNb[Face]
    = ut_realloc_1d_int ((*pPolymod).FaceVerNb[Face], (*pPolymod).FaceVerQty[Face] + 1);
