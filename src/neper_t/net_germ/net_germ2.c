...
void
net_germ_randomize (struct IN In, struct GERMSET* pGermSet, struct POLY Domain, char* morpho)
{
  int i, j;
  double eps; 
  double* r = ol_r_alloc ();
  ...
  if (In.randomize > 0)
  {
	...
	if (dir[j] == 1)
	{
	  (*pGermSet).GermsCoo[i][j] += eps * r[j - 1] * (bbox[2 * j] - bbox[2 * j - 1]);
	  if ((*pGermSet).GermsCoo[i][j] < bbox[2 * j - 1])
	    (*pGermSet).GermsCoo[i][j] = bbox[2 * j - 1] + 1e-6;
	  if ((*pGermSet).GermsCoo[i][j] > bbox[2 * j])
	    (*pGermSet).GermsCoo[i][j] = bbox[2 * j] - 1e-6;
	}
    }
