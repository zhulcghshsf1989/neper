 ...
 */
int
FaceEdges2PolyEdges (struct TESS *pTess, int PNb, int FNb, int qty)
{
  int i;
  int ENb;
   ...
   */
  for (i = 1; i <= (*pTess).FaceVerQty[FNb]; i++)
  {
    /* The edge number is recorded
     */
    ENb = (*pTess).FaceEdgeNb[FNb][i];
    /* If the edge has not been recorded as belonging to the
     * polyhedron yet, it is recorded.
     */
    if (oneDIntEltPos ((*pTess).PolyEdgeNb[PNb], 1, qty, ENb, 0) == -1)
    {
      /* the qty of polyedron faces is incremented.
       */
      qty++;
      (*pTess).PolyEdgeNb[PNb] = ut_realloc_1d_int ((*pTess).PolyEdgeNb[PNb], qty + 1);
      /* The polyhedron new edge number is recorded.
       */
      (*pTess).PolyEdgeNb[PNb][qty] = ENb;
    }
  }
