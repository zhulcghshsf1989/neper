...
UpdateFaceEdge (struct TESS *pTess, int edge, int verbosity)
{
  ...
  for (i = 0; i < (*pTess).EdgeFaceQty[edge]; i++)
  {
    ...
    if ((*pTess).FaceVerQty[face] >= 4)
      DeleteEdgeFromFace (pTess, face, edge, verbosity);
    else
	       ...
	       int verbosity)
{
  ...
  array =
    ut_array_1d_int_cat ((*pTess).VerEdgeNb[newver], (*pTess).VerEdgeQty[newver],
		(*pTess).VerEdgeNb[delver], (*pTess).VerEdgeQty[delver]);
  ...
  (*pTess).VerEdgeNb[newver] = ut_realloc_1d_int ((*pTess).VerEdgeNb[newver], qty);
...
UpdateVerBound (struct TESS *pTess, int delver, int newver)
{
  ...
  else if ((*pTess).VerDom[delver][0] == 0 || (*pTess).VerDom[newver][0] == 0)
  {
    ...
    if ((*pTess).VerDom[delver][0] == 0)
      ut_array_1d_int_memcpy ((*pTess).VerDom[newver], 2, (*pTess).VerDom[delver]);
  }
...
UpdateVerCooMiniFF (struct TESS *pTess, int newver, int verbosity)
{
  ...
  if (verbosity > 0)
  {
    printf ("newver = %d has bound: %d %d\n", newver, (*pTess).VerDom[newver][0], (*pTess).VerDom[newver][1]); 
