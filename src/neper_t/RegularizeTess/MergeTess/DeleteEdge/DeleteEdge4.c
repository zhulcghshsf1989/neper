...
void
DeleteEdgeFromFace (struct TESS *pTess, int face, int edge, int verbosity)
{
  int i;
  int pos;
...
DeleteFace (struct TESS *pTess, int face, int edge, int verbosity)
{
   ...
   */
  (*pTess).EdgeFaceNb[nnew] = ut_realloc_1d_int ((*pTess).EdgeFaceNb[nnew], (*pTess).EdgeFaceQty[nnew] + (*pTess).EdgeFaceQty[old]);
  ut_array_1d_int_memcpy ((*pTess).EdgeFaceNb[nnew] + (*pTess).EdgeFaceQty[nnew],
  ...
  for (i = 0; i < (*pTess).EdgeFaceQty[old]; i++)
  {
    ...
    pos =
      oneDIntEltPos ((*pTess).FaceEdgeNb[tmpface], 1,
		     (*pTess).FaceVerQty[tmpface], old, 0);
	...
	(*pTess).FaceEdgeNb[tmpface][k] = (*pTess).FaceEdgeNb[tmpface][k + 1];
      for (k = 1; k <= (*pTess).FaceVerQty[tmpface] - 1; k++)
	(*pTess).FaceEdgeOri[tmpface][k] = (*pTess).FaceEdgeOri[tmpface][k + 1];
      for (k = 1; k <= (*pTess).FaceVerQty[tmpface] - 1; k++)
    				      ...
    				      // should not have a domain edge
  {
    if (verbosity >= 3)
    {
      ...
      printf ("    %d %d becomes %d %d\n", (*pTess).EdgeDom[nnew][0],
		(*pTess).EdgeDom[nnew][1], (*pTess).EdgeDom[old][0], (*pTess).EdgeDom[old][1]);
    }
