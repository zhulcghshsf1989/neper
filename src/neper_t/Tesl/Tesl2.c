 ...
 */
void
TessGenDet (struct GERMSET GermSet, struct POLY *Poly, struct TESL *pTesl)
{
  (*pTesl).Type = ut_alloc_1d_char (10);
  strcpy ((*pTesl).Type, "standard"); 

  (*pTesl).Id = GermSet.Id;
  (*pTesl).morpho = ut_realloc_1d_char ((*pTesl).morpho, 10);

  strcpy ((*pTesl).morpho, GermSet.morpho);

  (*pTesl).PolyQty = TPolyQty (GermSet);
  (*pTesl).FaceQty = TFaceQty (GermSet, Poly);
  (*pTesl).VerQty = TVerQty (GermSet, Poly);
  (*pTesl).EdgeQty = TEdgeQty (GermSet, Poly);
...
TessAlloc (struct TESL *pTesl)
{
  ...
  (*pTesl).VerGerm = ut_alloc_2d_int ((*pTesl).VerQty + 1, 4);
  (*pTesl).VerCoo = ut_alloc_2d ((*pTesl).VerQty + 1, 3);
  (*pTesl).CenterCoo = ut_alloc_2d ((*pTesl).PolyQty + 1, 3);
...
IniTessVer (struct TESL *pTesl, struct POLY *Poly, int **TVNb)
{
  ...
  IniTessVerGerm (pTesl, Poly, TVNb);
...
void
IniTessPolyCenNXQty (struct GERMSET GermSet, struct TESL *pTesl,
		     struct POLY *Poly)
{
  int num;
  int i;
  ...
  {
    for (i = 0; i < 3; i++)
      (*pTesl).CenterCoo[num][i] = GermSet.GermsCoo[num][i + 1];
...
IniTessVernEdge (struct TESL *pTesl)
{
  ...
  for (i = 1; i <= (*pTesl).VerQty; i++)
  {
    ...
    {
      (*pTesl).VerOfDomain[0]++;
      (*pTesl).VerOfDomain = ut_realloc_1d_int ((*pTesl).VerOfDomain, (*pTesl).VerOfDomain[0] + 1);
