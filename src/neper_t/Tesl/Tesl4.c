 ...
 */
int
CommonGerms (struct TESL Tesl, int first, int second, int *nb)
{
  int i, j;
  int qty = 0;
  ...
  for (i = 0; i <= 3; i++)
    for (j = 0; j <= 3; j++)
      if (Tesl.VerGerm[first][i] == Tesl.VerGerm[second][j])
      {
	nb[qty] = Tesl.VerGerm[first][i];
	qty++;
      }
...
BissFace (struct TESL Tesl, int S1, int S2)
{
  ...
  for (i = 1; i <= Tesl.PolyFaceQty[S1]; i++)
  {
    ...
    if (oneDIntEltPos (Tesl.FacePoly[f], 0, 1, S2, 0) != -1)
      return f;
  }
