		   ...
		   png_infop * pinfo_ptr, png_bytep ** prow_pointers)
{
  ...
  /* allocation */
  (*prow_pointers) =
    (png_bytep *) malloc (sizeof (png_bytep) * (*pinfo_ptr)->height);
  for (y = 0; y < (*pinfo_ptr)->height; y++)
    (*prow_pointers)[y] = (png_byte *) malloc ((*pinfo_ptr)->rowbytes);
               ...
               png_bytep**  prow_pointers, char *file_name)
{
  ...
  for (i = 0; i < height; i++)
    (*prow_pointers)[i] = (png_byte *) malloc ((*pinfo_ptr)->rowbytes);
...
void
ut_png_free_png_file (png_infop info_ptr, png_bytep * row_pointers)
{
  unsigned int y;
